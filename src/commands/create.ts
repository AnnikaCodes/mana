...
import { channelMarkets, getAPIInstance } from '../common';
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option => option
        .setName('question')
        .setDescription('the question/title of the market; required if not in a channel with a default market')
        .setRequired(true)
    )
    .addStringOption(option => option
        .setName('description')
    ...
    .addIntegerOption(option => option
        ...
        .setDescription('the initial probability of the market - required for binary markets only')
    )
    .addIntegerOption(option => option
        .setName('min')
        .setDescription('the minimum value of the market - required for numeric markets only')
    )
    .addIntegerOption(option => option
        .setName('max')
        .setDescription('the maximum value of the market - required for numeric markets only')
    )
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (closeTime < Date.now() || !closeTime) {
        await interaction.editReply('You must specify a valid closing date (YYYY-MM-DD)');
        return;
    }

    const type = interaction.options.getString('type') as 'BINARY' | 'FREE_RESPONSE' | 'NUMERIC';
    const initialProb = interaction.options.getInteger('initial_probability');
    if (initialProb && type !== 'BINARY') {
        await interaction.editReply('Intial probabilities can only be specified for binary markets.');
        return;
    }
    ...
    if ((min || max) && type !== 'NUMERIC') {
        await interaction.editReply('Minimum and maximum values can only be specified for numeric markets.');
        return;
    }
    if (type === 'NUMERIC' && (!min || !max)) {
        await interaction.editReply('You must specify both a minimum and maximum value for numeric markets.');
        return;
    }
