...
import { channelMarkets, getMarketByID, getMarketByTitle } from '../common';
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option => option
        .setName('query')
        .setDescription('name of the market; required if not in a channel with a default market')
    );
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!query && interaction.channel && channelMarkets[interaction.channel.id]) {
        await interaction.reply(`Getting market info for <#${channelMarkets[interaction.channel.id]}>`);
        market = getMarketByID(channelMarkets[interaction.channel.id]);
    } else {
        await interaction.reply(`Searching for ${query}...`);
        market = await getMarketByTitle(query);
    }
    if (!market) return interaction.editReply(query ? `No market matched ${query}` : 'No market found for this channel - specify');
    ...
    switch (market.outcomeType as string) {
        case 'BINARY':
            ...
            break;
        case 'FREE_RESPONSE':
            replies.push(`Answers: ${market.answers?.map(a => `"${a.text}" (${(a.probability * 100).toFixed(0)}%)`).join(', ')}`);
            break;
        default:
            replies.push(`${market.outcomeType} markets, like this one, are not fully supported`);
            break;
    }
